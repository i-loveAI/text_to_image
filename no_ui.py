...
def generate(poseFile, shapeText, textureText, step=10):
    ...
    result=sample_model.sample_and_refine(step)
    result=result.permute(0, 2, 3, 1)
    result=result.detach().cpu().numpy()
    result*=255
    result=np.asarray(result[0, :, :, :], dtype=np.uint8)
    save_path='./Compare/'
    image_name=save_path+'Result'+str(step)+".png"
...
if __name__ == '__main__':
    ...
    for step in range(10, 301, 5):
        set_random_seed(100)
        print("Step: ", step)
        generate(poseFile, shapeText, textureText, step)
    print("Complete!")
